...
import { saveGuestCurrent, readGuestCurrent, clearGuestCurrent, type GuestRecord, type GuestEvidenceItem } from "@/lib/guest-storage";
import { generateReportPDF, downloadPDF } from "@/lib/report-pdf";
import { RISK_META } from "@/lib/scoring";
import {
  ShieldCheck, ArrowLeft, Loader2, Save, Sparkles, CheckCircle2, XCircle, AlertTriangle,
  Info, Trash2, LogIn, Upload, Image as ImageIcon, FileText, Mail, Type, Link2, Download,
  X, Paperclip, Brain, TargetIcon, PlayCircle,
} from "lucide-react";
...
function GuestInvestigate() {
  ...
  const [evidence, setEvidence] = useState<Array<GuestEvidenceItem & { status: UploadStatus; error?: string }>>([]);
  ...
  const addFiles = useCallback(async (files: FileList | File[]) => {
    ...
    for (const file of list) {
      ...
      if (!kind) { toast.error(`Unsupported file type: ${file.name}`); continue; }
      const previewUrl = kind === "image" ? await fileToDataURL(file) : undefined;
      const placeholder: GuestEvidenceItem & { status: UploadStatus } = {
        id, kind, filename: file.name, extracted_text: "", channel: "unknown",
        urls: [], emails: [], phones: [], people: [], companies: [], amounts: [],
        payment_methods: [], red_flag_notes: [], preview_data_url: previewUrl,
        original_size: file.size, status: "extracting",
      };
      ...
      try {
        ...
        setEvidence((prev) => prev.map((e) => e.id === id ? { ...e, ...extracted, status: "ready" } : e));
        if (!caseName) {
          const guess = extracted.companies[0] || extracted.people[0] || file.name;
          setCaseName(guess.slice(0, 100));
        }
      } catch (err) {
        setEvidence((prev) => prev.map((e) => e.id === id ? { ...e, status: "error", error: (err as Error).message } : e));
  ...
  const addFreeText = async () => {
    ...
    const id = crypto.randomUUID();
    const placeholder: GuestEvidenceItem & { status: UploadStatus } = {
      id, kind: "text", filename: "", extracted_text: "", channel: "unknown",
      urls: [], emails: [], phones: [], people: [], companies: [], amounts: [],
      payment_methods: [], red_flag_notes: [], status: "extracting",
    };
    setEvidence((prev) => [...prev, placeholder]);
    try {
      const extracted = await extractFn({ data: { kind: "text", filename: "", payload: text, mime_type: "text/plain" } }) as ExtractedEvidence;
      setEvidence((prev) => prev.map((e) => e.id === id ? { ...e, ...extracted, status: "ready" } : e));
    } catch (err) {
      setEvidence((prev) => prev.map((e) => e.id === id ? { ...e, status: "error", error: (err as Error).message } : e));
    }
  ...
  const hasAnyEvidence = evidence.length > 0 || freeUrl.trim() || freeEmail.trim() || freeText.trim();
  ...
  const run = async () => {
    ...
    setRunning(true);
    try {
      const urls = freeUrl.split(/[\s,;]+/).map(s => s.trim()).filter(Boolean);
      const emails = freeEmail.split(/[\s,;]+/).map(s => s.trim().toLowerCase()).filter(Boolean);
      // include any typed free text as an inline evidence item too
      const evidencePayload: GuestEvidenceItem[] = readyEvidence.map(({ status, error, ...rest }) => rest);
      const result = await runFn({ data: { name: finalName, urls, emails, text: freeText.trim(), evidence: evidencePayload } }) as GuestResult;
      // Narrative + playbook match
      let narrative: Narrative | undefined;
      try {
        narrative = await narrateFn({ data: {
          case_name: finalName,
          ...
          evidence: evidencePayload.map((e) => ({
            kind: e.kind, filename: e.filename, extracted_text: e.extracted_text,
            channel: e.channel, urls: e.urls, emails: e.emails, phones: e.phones,
            payment_methods: e.payment_methods, red_flag_notes: e.red_flag_notes,
          })),
        }}) as Narrative;
      } catch (e) {
      ...
      const rec: GuestRecord = {
        id: crypto.randomUUID(), name: finalName, createdAt: new Date().toISOString(),
  ...
  const reset = () => {
    clearGuestCurrent();
    setRecord(null);
    setEvidence([]); setFreeUrl(""); setFreeEmail(""); setFreeText(""); setCaseName("");
  };
  ...
  const doSave = async () => {
    ...
    setClaiming(true);
    try {
      const { id } = await claimFn({ data: { name: record.name, input: { urls: record.input.urls, emails: record.input.emails, text: record.input.text }, result: record.result } });
      ...
      {dragOver && (
        <div className="fixed inset-0 z-40 grid place-items-center bg-primary/10 backdrop-blur-sm">
          <div className="rounded-2xl border-2 border-dashed border-primary/60 bg-background/80 px-10 py-8 text-center">
            ...
            <div className="text-xs text-muted-foreground">Screenshots · PDFs · Email files · Text</div>
          </div>
        </div>
      )}
      ...
      <main className="mx-auto max-w-5xl px-6 pb-24">
        {!record && (
          <div className="space-y-6">
            <div>
              <div className="mono text-xs uppercase tracking-[0.22em] text-primary/80">Investigation intake</div>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Drop anything you have.</h1>
              <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                A screenshot of the recruiter's DM. The offer letter PDF. The forwarded email. A link.
                Even just a WhatsApp text you copied. ANVIX reads all of it, cross-checks it live, and tells
                you what scam pattern it matches and what they'll ask you for next.
              </p>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <QuickInput icon={Link2} label="Add URL" placeholder="https://..." value={freeUrl} onChange={setFreeUrl} />
              <QuickInput icon={Mail} label="Add recruiter email" placeholder="alex@company.com" value={freeEmail} onChange={setFreeEmail} />
            ...
            {evidence.length > 0 && (
              <div className="glass rounded-2xl p-5">
                ...
                <ul className="space-y-2">
                  {evidence.map((ev) => <EvidenceRow key={ev.id} ev={ev} onRemove={() => removeEvidence(ev.id)} />)}
                </ul>
              </div>
            )}
            ...
            <div className="glass rounded-2xl p-5">
              <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
                <label className="block">
                  <div className="mb-1.5 text-xs font-medium text-muted-foreground">Case name (optional)</div>
                  <input className="input" placeholder="e.g. 'Google recruiter Alex — LinkedIn DM'" value={caseName} onChange={(e) => setCaseName(e.target.value)} />
                </label>
                <button
                  onClick={run} disabled={running || anyExtracting || !hasAnyEvidence}
              ...
              <p className="mt-2 text-xs text-muted-foreground">No account required to investigate. Sign in only when you want to download the PDF or save history.</p>
            </div>
          </div>
        )}

        {record && <ReportView record={record} onReset={reset} onSave={doSave} onDownload={doDownload} saving={claiming} downloading={downloading} />}
      </main>

      {authOpen && <AuthPrompt intent={authOpen} onClose={() => setAuthOpen(null)} onAuthed={() => { const i = authOpen; setAuthOpen(null); setTimeout(() => i === "download" ? doDownload() : doSave(), 200); }} />}
    </div>
  );
}
...
function QuickInput({ icon: Icon, label, placeholder, value, onChange }: { icon: React.ComponentType<{ className?: string }>; label: string; placeholder: string; value: string; onChange: (s: string) => void }) {
  return (
...
function QuickPasteText({ value, onChange, onSubmit }: { value: string; onChange: (s: string) => void; onSubmit: () => void }) {
  return (
    <div className="glass rounded-xl p-3">
      <div className="mb-1.5 flex items-center gap-2 text-xs font-medium text-muted-foreground">
        <Type className="h-3.5 w-3.5" /> Paste message / description
      </div>
      <textarea className="input min-h-[46px] resize-none" placeholder="Paste the WhatsApp thread, DM, or JD text…" value={value} onChange={(e) => onChange(e.target.value)} />
      {value.trim().length > 0 && (
        <button onClick={onSubmit} className="mt-1.5 text-xs font-medium text-primary hover:underline">
...
function EvidenceRow({ ev, onRemove }: { ev: GuestEvidenceItem & { status: UploadStatus; error?: string }; onRemove: () => void }) {
  const Icon = ev.kind === "image" ? ImageIcon : ev.kind === "pdf" ? FileText : ev.kind === "eml" ? Mail : Type;
  return (
    <li className="flex items-start gap-3 rounded-lg border border-border/60 bg-surface/50 p-3">
      {ev.preview_data_url ? (
        <img src={ev.preview_data_url} alt="" className="h-12 w-12 shrink-0 rounded-md object-cover ring-1 ring-border" />
      ) : (
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] uppercase tracking-wide text-muted-foreground">{ev.kind}</span>
          {ev.channel !== "unknown" && <span className="text-[10px] text-muted-foreground">· {ev.channel}</span>}
        </div>
        {ev.status === "extracting" && (
          <div className="mt-1 flex items-center gap-1.5 text-xs text-primary"><Loader2 className="h-3 w-3 animate-spin" /> Reading with AI…</div>
        )}
        {ev.status === "error" && (
        ...
        {ev.status === "ready" && (
          <div className="mt-1 space-y-0.5 text-xs text-muted-foreground">
            {ev.people.length > 0 && <div>People: <span className="text-foreground">{ev.people.slice(0, 3).join(", ")}</span></div>}
            {ev.companies.length > 0 && <div>Companies: <span className="text-foreground">{ev.companies.slice(0, 3).join(", ")}</span></div>}
            {ev.emails.length > 0 && <div>Emails: <span className="text-foreground">{ev.emails.slice(0, 3).join(", ")}</span></div>}
            {ev.phones.length > 0 && <div>Phones: <span className="text-foreground">{ev.phones.slice(0, 3).join(", ")}</span></div>}
            {ev.payment_methods.length > 0 && <div className="text-destructive/90">Payment methods: {ev.payment_methods.join(", ")}</div>}
            {ev.red_flag_notes.length > 0 && <div className="text-warning">⚑ {ev.red_flag_notes[0]}</div>}
          </div>
        )}
      </div>
      <button onClick={onRemove} className="rounded-md p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground">
...
function ReportView({ record, onReset, onSave, onDownload, saving, downloading }: { record: GuestRecord; onReset: () => void; onSave: () => void; onDownload: () => void; saving: boolean; downloading: boolean }) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-1 text-xs text-muted-foreground">Ran {new Date(record.createdAt).toLocaleString()} · guest session · {record.input.evidence.length} evidence item(s)</div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={onDownload} disabled={downloading} className="inline-flex items-center gap-2 rounded-md border border-border bg-surface px-3 py-2 text-sm hover:bg-accent disabled:opacity-50">
      ...
      <div className="glass rounded-2xl p-6">
        <div className="flex flex-wrap items-center justify-between gap-6">
          ...
          <div className="grid gap-2 text-sm">
            <Stat label="ML fraud probability" value={`${(r.fraud_probability * 100).toFixed(1)}%`} />
        ...
        {n?.headline && <div className="mt-5 rounded-lg border border-primary/30 bg-primary/5 p-4 text-base font-medium">{n.headline}</div>}
        <p className="mt-4 text-sm leading-relaxed text-muted-foreground">{n?.narrative || r.summary}</p>
        ...
        <div className="glass overflow-hidden rounded-2xl border border-warning/30 bg-warning/5">
          ...
          <div className="p-6">
            <div className="flex flex-wrap items-baseline justify-between gap-3">
              ...
              <div className="text-xs text-muted-foreground">Confidence {(n.playbook.confidence * 100).toFixed(0)}%{n.playbook.current_step_index !== null ? ` · at step ${n.playbook.current_step_index}` : ""}</div>
            </div>
            {n.playbook.matched_signals.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1.5">
                {n.playbook.matched_signals.map((s, i) => (
                  <span key={i} className="rounded-full bg-warning/15 px-2 py-0.5 text-[11px] text-warning">{s}</span>
                ))}
              </div>
            )}
              ...
              <div className="mt-5">
                ...
                <ol className="mt-2 space-y-2">
                  {n.next_predicted_asks.map((a, i) => (
                    <li key={i} className="flex items-start gap-3 rounded-md border border-border/60 bg-surface/60 p-3 text-sm">
                      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-warning/20 text-xs font-semibold text-warning">{i + 1}</span>
                      <span>{a}</span>
                    </li>
                  ))}
                </ol>
            ...
            {n.playbook.what_to_do && (
              <div className="mt-4 rounded-md border border-warning/30 bg-warning/10 p-3 text-sm">
                <span className="font-medium text-warning">What to do: </span>{n.playbook.what_to_do}
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {n?.key_evidence?.length ? (
          <div className="glass rounded-2xl p-6">
            <div className="mb-3 flex items-center gap-2 text-sm font-medium"><Brain className="h-4 w-4 text-primary" /> Key evidence cited</div>
            <ul className="space-y-2 text-sm">
              {n.key_evidence.map((e, i) => (
                <li key={i} className="flex items-start gap-2"><span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" /><span>{e}</span></li>
              ))}
            </ul>
          </div>
        ) : (
        ...
        {n?.action_checklist?.length ? (
          <div className="glass rounded-2xl p-6">
            <div className="mb-3 flex items-center gap-2 text-sm font-medium"><PlayCircle className="h-4 w-4 text-primary" /> Do this now</div>
            <ol className="space-y-2 text-sm">
              {n.action_checklist.map((a, i) => (
                <li key={i} className="flex items-start gap-2"><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-primary/15 text-xs text-primary">{i + 1}</span><span>{a}</span></li>
              ))}
            </ol>
          </div>
        ) : (
      ...
      {n?.disclaimer && <div className="text-center text-xs text-muted-foreground">{n.disclaimer}</div>}
    </div>
  );
}
...
function VItem({ v }: { v: { category: string; check_name: string; status: string; score: number | null; detail: string } }) {
  const Icon = v.status === "pass" ? CheckCircle2 : v.status === "fail" ? XCircle : v.status === "warning" ? AlertTriangle : Paperclip;
...
function AuthPrompt({ intent, onClose, onAuthed }: { intent: "save" | "download"; onClose: () => void; onAuthed: () => void }) {
  ...
  const title = intent === "download" ? "Sign in to download your report" : "Sign in to save this investigation";
  const sub = intent === "download"
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 p-4 backdrop-blur">
      <div className="glass w-full max-w-sm rounded-2xl p-6">
        <div className="flex items-center gap-2 text-sm font-medium"><LogIn className="h-4 w-4 text-primary" /> {title}</div>
        ...
        <div className="mt-4 space-y-3">
          ...
          <button onClick={submit} disabled={busy || !email || !password} className="w-full rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">
            {busy ? <Loader2 className="mx-auto h-4 w-4 animate-spin" /> : mode === "signin" ? "Sign in & continue" : "Create account & continue"}
          </button>
