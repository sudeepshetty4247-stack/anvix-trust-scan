...
export function StartInvestigationDialog({
  ...
  function close() {
    if (busy) return;
    setName(""); setUrlInput(""); setTextInput(""); setItems([]);
    onOpenChange(false);
  }
  ...
  async function start() {
    if (!name.trim()) { toast.error("Give your investigation a name."); return; }
    if (items.length === 0) { toast.error("Add at least one piece of evidence."); return; }
    setBusy(true);
    try {
      ...
      // Upload files first
      const uploads = await Promise.all(items.map(async (it) => {
      ...
      const payload = items.map((it, i) => {
        if (it.kind === "url") return { kind: "url" as const, content: it.content, label: it.content };
        if (it.kind === "text") return { kind: "text" as const, content: it.content, label: "Text note" };
        const u = uploads[i]!;
        return {
          kind: "file" as const, label: u.file.name, content: u.file.name,
          storage_path: u.path, mime_type: u.file.type || null as unknown as string, size_bytes: u.file.size,
        };
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/70 p-4 backdrop-blur-sm" onClick={close}>
      <div className="glass w-full max-w-xl overflow-hidden rounded-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
          <div>
            <div className="mono text-[10px] uppercase tracking-widest text-primary">New investigation</div>
            <div className="text-sm text-muted-foreground">Name it, drop evidence, and ANVIX takes over.</div>
          </div>
          <button onClick={close} className="rounded p-1 text-muted-foreground hover:bg-accent hover:text-foreground"><X className="h-4 w-4" /></button>
        </div>
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          >
            ...
            <div className="mono mt-1 text-[11px] text-muted-foreground">PDF · Images · Docs · Screenshots · Emails</div>
          ...
          {items.length > 0 && (
            <div className="max-h-40 space-y-1.5 overflow-auto rounded-md border border-border/60 p-2">
              {items.map((it, i) => (
                <div key={i} className="flex items-center justify-between rounded bg-surface/60 px-2 py-1.5 text-xs">
                  ...
                  <button onClick={() => setItems((s) => s.filter((_, k) => k !== i))} className="ml-2 text-muted-foreground hover:text-destructive">
        ...
        <div className="flex items-center justify-between border-t border-border px-5 py-3">
          <div className="mono text-[11px] text-muted-foreground">{items.length} evidence item(s)</div>
          <div className="flex gap-2">
            <button onClick={close} className="rounded-md px-3 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground" disabled={busy}>
              Cancel
            </button>
            <button
